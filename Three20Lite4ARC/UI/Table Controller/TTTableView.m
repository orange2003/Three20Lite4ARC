...
- (CAGradientLayer*)shadowAsInverse:(BOOL)inverse
{
    ...
    CGColorRef darkColor = [UIColor colorWithRed:0.0
                                           green:0.0
                                            blue:0.0
                                           alpha:inverse ? (kShadowInverseHeight / kShadowHeight) * 0.5
                                                         : 0.5].CGColor;
    CGColorRef lightColor = [self.backgroundColor
                                colorWithAlphaComponent:0.0].CGColor;

    newShadow.colors = @[
        (id)CFBridgingRelease(inverse ? lightColor : darkColor),
        (id)CFBridgingRelease(inverse ? darkColor : lightColor)
    ];
    return newShadow;
}
...
- (void)layoutSubviews
{
    ...
    if (nil == _originShadow) {
        ...
        [self.layer insertSublayer:_originShadow
                           atIndex:0];
    ...
    if (0 == [firstRow section]
        ...
        if (nil == _topShadow) {
            ...
            [cell.layer insertSublayer:_topShadow
                               atIndex:0];
    ...
    if ([lastRow section] == [self numberOfSections] - 1
        ...
        if (nil == _bottomShadow) {
            ...
            [cell.layer insertSublayer:_bottomShadow
                               atIndex:0];
